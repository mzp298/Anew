...
\begin{equation}
\alpha=1-\left(1-\dfrac{1}{s_{min}} \right)^a  .
\label{alpha}
\end{equation}
...
\subsection{One dimensional application to simple cyclic data(from Cetim)}
...
$$\alpha=1-\left(1-\dfrac{S_{max}}{\sigma_y\left( 1-\lambda\Sigma_{H}\right) } \right)^a.$$
...
\subsection{Parameter identification on 30NCD16 steel}
...
\end{table}
Applying $W_{cyc}=\dfrac{4(E-k)(1+\nu)\left( \beta-1\right) }{ E(E+k\nu)\beta\left( \beta+1\right) }\dfrac{S_{max}^{\beta+1}}{ \left(\sigma_y-\lambda \Sigma_H\right)^{\beta-1}}$ to pure bending and torsion test we can  find the value of $W_F$ and $\beta$ as depicted in \figref{30NCD60noH}.
\begin{figure}[!h]
	\centering
...
The stress of $Ep_{01}$ and $Ep_{02}$ are respectively $131.9MPa$ and $97.0MPa$. We found the same parameters feasible to both loading cases.
\begin{table}[!h]
	\centering
	\begin{tabular}{ll}
		\hline
		\textbf{Parameters}                                         & \textbf{Value}                    \\ \hline
		Load                                                              & $\Sigma=131.9sin(t)\&97sin(t)$ MPa                  \\
		Young's modulus                                             & $E=72$ GPa                       \\
		Hardening parameter                                         &  $k=6$ MPa \\
		Hydrostatic pressure sensitivity                            & $\lambda=0$                     \\
		Macroscopic yield stress                                    & $\sigma_y=230$ MPa              \\
		Material parameter from Chaboche law(Wohler curve exponent) & $\gamma=6$                        \\
		Non-linearity of damage accumulation & $\alpha=1-\left(1-\dfrac{S_{max}}{\sigma_y\left( 1-\lambda\Sigma_{H}\right) } \right)^2$                        \\
	 	Weakening scales distribution exponent                      & $\beta=3.928$                             \\
		Dissipated energy to failure per unit volume                & $W_F=3.219$ GJ                       \\   \hline
	\end{tabular}
	\caption{Material parameters in a simple cyclic load }
	\label{Sin}
\end{table}
\begin{Figure}[h!]{Damage evolution of aluminum alloy(sample 1 and 2) at constant loading with $R=-1$.}[]
	\graphfile*[30]{figures//ep01_damage.png}[]
	\graphfile*[30]{figures//ep02_damage.png}[]
	\label{ep0102}
\end{Figure}
...
\begin{table}[!h]
	...
	\begin{tabular}{cccc}
		\hline
		 &   $Ep\_{03}$&  $Ep\_{04}\_1$ & $Ep\_{04}\_2$  \\ \hline
		 		$\lambda$      &0.3                  & 0.3    & 0.3             \\ \hline
		$n_0$      &254                  & 12    & 13                 \\ \hline
	\end{tabular}
	\label{tab:ep0304}
\end{table}
\begin{Figure}[h!]{Damage evolution of aluminum alloy(sample 3 and 4) at random loading.}[]
	\graphfile*[30]{figures//ep03_stress.png}[]
	\graphfile*[30]{figures//ep03_damage.png}[]
	\\
	\graphfile*[30]{figures//ep04_1_stress.png}[]
    \graphfile*[30]{figures//ep04_1_damage.png}[]
	\\
    \graphfile*[30]{figures//ep04_2_stress.png}[]
    \graphfile*[30]{figures//ep04_2_damage.png}[]
	\label{ep0304}
\end{Figure}
