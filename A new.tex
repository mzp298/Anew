...
\begin{equation}
\delta [1-(1-D)^{\gamma+1}]^{1-\alpha}=n_0(1-\alpha)(\gamma+1)\dfrac{\dot{W}\delta t}{W_F}.
\label{dt}
\end{equation}

In a general loading case, $\dot{W}$ is the microscopic energy dissipation rate of unit defect. By integrating Eq.\eqref{dissipated} over all microscales, we get:
...
\begin{equation}
\alpha=1-\dfrac{\dot{W}dt}{\sigma_u\left(1-\dfrac{1}{s_{min}} \right) }.
\label{alpha}
\end{equation}
Where $s_{min}$ is the minimum scale that causes energy loss, meaning the deviatoric stress is greater than local yield stress: $$S_{max}>\dfrac{\sigma_y}{s_{min}},$$
$$s_{min}=\dfrac{\sigma_y}{S_{max}}.$$
which will change over time in loading history, we can now use numerical method to verify our analysis in 1-D cyclic loading.
...
$$\left( \uline{\uline{S}}-\uline{\uline{b}}\right) (t+dt)\leqslant \sigma_y/s.$$
...
\left| \left|\uline{\uline{S}}-\uline{\uline{b}}\right| \right|= \sigma_y/s, & yield   limit,\\
...
with $$\eta=max\left\lbrace \underbrace{0}_{elastic\; regime}, \underbrace{\dfrac{\left| \left|\uline{\uline{S}}-\uline{\uline{b}}\right| \right|_{trial}}{ \sigma_y/s}-1}_{plastic \; regime\; when\; this\; number\; is\; positive}\right\rbrace, $$
...
\begin{equation}
\dot{w}(s)=\left( \uline{\uline{S}}-\uline{\uline{b}}\right) :\dot{\uline{\uline{\varepsilon}}}^p=\gamma\dfrac{  \sigma_y}{s}.
\label{w}
\end{equation}

From Eq.\eqref{eta} and Eq.\eqref{eta2} in annex, we get:

\begin{equation}E\gamma dt=\left\langle \left| \left|\uline{\uline{S}}-\uline{\uline{b}}\right| \right|_{trial}-\dfrac{ \sigma_y}{s}\right\rangle /\left(\dfrac{1}{1+\nu}+\dfrac{k}{E-k} \right)=\left\langle \left| \left|\uline{\uline{S}}-\uline{\uline{b}}\right| \right|_{trial}-\dfrac{ \sigma_y}{s}\right\rangle\dfrac{(E-k)(1+\nu) }{(E+k\nu)},
\label{gamma}
\end{equation}
...
\begin{equation}
\dot{w}(s)dt=\dfrac{(E-k)(1+\nu) }{E(E+k\nu)}\left\langle  \left| \left|\uline{\uline{S}}-\uline{\uline{b}}\right| \right|_{trial}-\dfrac{ \sigma_y}{s}\right\rangle \dfrac{ \sigma_y}{s}.
\label{dW}
\end{equation}
...
\\&=\dfrac{(E-k)(1+\nu) }{2E(E+k\nu)}\sum_{i}\omega_i\left\langle  \left| \left|\uline{\uline{S}}-\uline{\uline{b}}\right| \right|_{trial}-\dfrac{\sigma_y M(\sigma_H)}{\left( \dfrac{x_i+1}{2}\right) ^{\frac{1}{1-\beta}}}\right\rangle \dfrac{\sigma_y M(\sigma_H)}{\left( \dfrac{x_i+1}{2}\right)^{\frac{1}{1-\beta}}}.
\end{split}
\label{finaldw}
\end{equation}
...
We use matlab to realize our analytical method. We plot $\left\|  \uline{\uline{S}}-\uline{\uline{b}}\right\|_{trial}$ and $\left\|  \uline{\uline{S}}-\uline{\uline{b}}\right\|$ at two different scales($s_33=4.20$ and $s_40=9.68$) in \figref{trialsin}.
\begin{figure}[!h]
	\centering
...
\begin{table}[!h]
	...
	\begin{tabular}{|c|c|c|c|}
		\hline
		\textbf{\begin{tabular}[c]{@{}c@{}}Tests\\ {(}R=-1{)}\end{tabular}} & \textbf{\begin{tabular}[c]{@{}c@{}}N\\ {[}Cycles{]}\end{tabular}} & \textbf{\begin{tabular}[c]{@{}c@{}}Pure bending\\ {[}MPa{]}\end{tabular}} & \textbf{\begin{tabular}[c]{@{}c@{}}Pure torsion\\ {[}MPa{]}\end{tabular}} \\  \hline
		\textbf{1}     & 51000                                                             & 820             &527                                                                                   \\ \hline
		\textbf{2}     & 80000                                                             & 795              &505                                                                                  \\ \hline
		\textbf{3}     & 90000                                                             & 790               &  500                                                                               \\ \hline
		\textbf{4}     & 95000                                                             & 785                &   497                                                                             \\ \hline
		\textbf{5}     & 100000                                                            & 780                &   495                                                                             \\ \hline
		\textbf{6}     & 120000                                                            & 765                &   482                                                                             \\ \hline
		\textbf{7}     & 140000                                                            & 752                 &   470                                                                            \\ \hline
...
\end{table}
Applying $W_{cyc}=\dfrac{4(E-k)(1+\nu)\left( \beta-1\right) }{ E(E+k\nu)\beta\left( \beta+1\right) }\dfrac{S_{max}^{\beta+1}}{ \sigma_y^{\beta-1}}$ to pure bending and torsion test we can  find the value of $W_F$ and $\beta$ as depicted in \figref{30NCD60noH}.
\begin{figure}[!h]
	\centering
...
In the load history, when $\left\|  \uline{\uline{S}}-\uline{\uline{b}}\right\|_{trial}>\sigma_y$, the damage accumulates. However, at scale $s_{8}$, there are much less damage accumulation than at scale $s_1$.  In this way we do not neglect the small influences in load history and the big fluctuation in stress is magnified which reflects the real situation. 
